...
FILEPATH="${SCRIPT_DIR}/../array_s3_to_bigquery"

help() {
echo "
  Type:
  (例) ./array_s3_to_bigquery テーブル名 カラム名(カンマ区切り) CSV名
  (例) ./array_s3_to_bigquery account id,user_id account.csv
    "
}
...
_export:
  s3_target_date: \${moment(session_date).format('YYYY-MM-DD')}
  bq_target_date: \${moment(session_date).format('YYYYMMDD')}
_error:
  sh>: export \$(cat config/env | xargs) && /digdag_plaza/post_slack.sh "[\${session_time}][\${session_id}] DigDag Fail ${TABLENAME}"
+go:
  sh>: export \$(cat config/env | xargs) && /go/bin/data-transfer -t ${TABLENAME}
+load:
...
in:
  type: s3
  path_prefix: salesforce/{{ env.s3_target_date }}/$TABLENAME.json
...
out:
  ...
  json_keyfile: /digdag_plaza/array_s3_to_bigquery/config/bq.key
  ...
  dataset: source__sfdc_paraft
  table: ${TABLENAME}
  schema_file: /digdag_plaza/array_s3_to_bigquery/embulk/db/$TABLENAME.json
